...
exports.claimBookingReview = (0, https_1.onCall)({ region: "asia-southeast1" }, async (request) => {
    const data = request.data;
    const bookingId = String(data?.bookingId ?? "").trim();
    const token = String(data?.token ?? "").trim();
    if (!bookingId) {
    ...
    if (!SERVED_STATUSES.has(String(b.status ?? ""))) {
        throw new https_1.HttpsError("failed-precondition", "This reservation hasn't been completed yet.");
    }
    ...
    const customToken = ownerOk
        ? null
        : await (0, auth_1.getAuth)().createCustomToken(`bkgreview_${bookingId}`, {
