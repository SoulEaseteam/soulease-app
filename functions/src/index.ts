...
//
//   Deliberately local rather than importing telegram-concierge-bot/client.ts:
//   that client always sets parse_mode: "HTML", and our job DM contains a
...
async function answerCallback(
  ...
  try {
    ...
    logger.warn("[answerCallback] failed", err);
  }
}
...
function galleryChangeLine(before: unknown, after: unknown): string {
  const b = Array.isArray(before) ? before.length : 0;
  const a = Array.isArray(after) ? after.length : 0;
  if (a < b) return `ลบรูป ${b - a} รูป · เหลือ ${a} รูป`;
  if (a > b) return `เพิ่มรูป ${a - b} รูป · รวม ${a} รูป`;
...
interface BookingDocLite {
  ...
  //   time (BookingFlowPage's effectiveTotalPrice); what was actually
  //   missing is that formatBookingForAdmin never showed the discount
  //   line, so "Price: 1,200" next to "Total: 1,000" with no line
  //   explaining the gap read as "the code didn't apply."
  discountCode?: string;
...
const HISTORY_TIER_THRESHOLDS: { emoji: string; label: string; visits: number; spend: number }[] = [
...
function normPhoneForHistory(raw: string): string {
  const digits = raw.replace(/\D/g, "");
  if (digits.startsWith("66") && digits.length >= 11) return "0" + digits.slice(2);
  return digits;
}

async function customerHistoryLine(
  ...
  try {
    ...
    snap.forEach((d) => {
      if (d.id === excludeBookingId) return; // history BEFORE this booking
      const bk = d.data() as {
        phone?: string;
        status?: string;
        totalPrice?: number;
      ...
      if (normPhoneForHistory((bk.phone ?? "").trim()) !== target) return;
      const st = bk.status ?? "";
      if (HISTORY_NOSHOW.has(st)) noShowCount++;
    ...
    logger.error("[customerHistoryLine] scan failed", err);
    return null;
  }
...
const formatBookingForAdmin = (
  ...
  const lines: (string | null)[] = [
    ...
    b.discountAmount && b.discountAmount > 0
      ? `🎟️ Discount: -${b.discountAmount.toLocaleString()} ฿${
  ...
  async (event) => {
        ...
        if (chatId) {
          ...
          if (therapistResult.ok) {
            await getFirestore()
              .collection("bookings")
              .doc(bookingId)
              .update({ dispatchDmSentAt: FieldValue.serverTimestamp() });
          }
        } else {
    ...
    if (!data.therapistId) {
      try {
        ...
        if (chan) {
          const posted = await sendTelegramIfEnabled(
            token,
            chan,
            formatOpenJob(bookingId, data),
            openJobKeyboard(bookingId)
          );
          await getFirestore().collection("telegramLogs").add({
            bookingId,
            ok: posted.ok,
          ...
          logger.info("[onBookingCreate] no job channel configured");
        }
      } catch (err) {
        // Same fail-open rule: the admin group already has the master copy.
        logger.error("[onBookingCreate] open-job post failed", err);
      }
    }
  }
);
...
async function linkTherapistByCode(
  code: string,
  chatId: number,
  greetName: string
): Promise<string> {
  ...
  if (!/^[A-Z0-9]{4,8}$/.test(code)) return bad;
  ...
  try {
    ...
    if (!exp || exp.toMillis() < Date.now()) return bad;
    ...
    logger.info("[linkTherapistByCode] linked", {
      therapistId: tDoc.id,
      chatId,
    });
    ...
    logger.error("[linkTherapistByCode] failed", err);
    return bad;
  }
}
...
async function isAdminChatId(chatId: number): Promise<boolean> {
  try {
    const snap = await getFirestore()
      .collection("adminSettings")
      .doc("advanced")
      .get();
    const ids = (snap.data()?.adminTelegramChatIds as string[] | undefined) ?? [];
    return ids.includes(String(chatId));
  } catch (err) {
    logger.warn("[isAdminChatId] lookup failed", err);
    return false;
  }
}
 ...
 */
async function linkAdminByCode(code: string, chatId: number): Promise<string | null> {
  const db = getFirestore();
  const ref = db.collection("adminSettings").doc("advanced");
  const snap = await ref.get();
  const d = snap.data() ?? {};
  const want = typeof d.adminLinkCode === "string" ? d.adminLinkCode : "";
  const exp = d.adminLinkCodeExpiresAt as Timestamp | undefined;
  if (!want || !exp || exp.toMillis() < Date.now()) return null;
  if (code.toUpperCase() !== want.toUpperCase()) return null;

  await ref.set(
    {
      adminTelegramChatIds: FieldValue.arrayUnion(String(chatId)),
      adminLinkCode: FieldValue.delete(),
      adminLinkCodeExpiresAt: FieldValue.delete(),
    },
    { merge: true }
  );
  logger.info("[linkAdminByCode] linked", { chatId });
  return [
...
const ADMIN_ORDER_SERVICES: Array<{
  id: string;
  name: string;
  keywords: string[];
  baseDuration: number;
...
function resolveServiceGuess(
  guess: string | null | undefined
): { id: string; name: string } | null {
  if (!guess) return null;
  const g = guess.toLowerCase();
  const match = ADMIN_ORDER_SERVICES.find((s) =>
    s.keywords.some((k) => g.includes(k))
  );
...
function priceForServiceDuration(
  ...
  const nearest = tiers.reduce((a, b) =>
...
const SERVICE_SPLIT_DEFAULTS_SERVER: Record<string, Record<number, number>> = {
...
async function resolveTherapistByName(
  nameGuess: string | null
): Promise<{ id: string; name: string } | null> {
  if (!nameGuess) return null;
  const g = nameGuess.trim().toLowerCase();
  if (!g) return null;
  const snap = await getFirestore().collection("therapists").get();
  let substringHit: { id: string; name: string } | null = null;
  for (const doc of snap.docs) {
    const name = (doc.data() as { name?: string }).name;
    if (!name) continue;
    const n = name.trim().toLowerCase();
    if (n === g) return { id: doc.id, name };
...
interface ParsedOrder {
  ...
  phone: string | null;
  therapistNameGuess: string | null;
  notes: string | null;
}
...
async function handleOrderPaste(
  text: string,
  chatId: number,
  token: string,
  apiKey: string
  ...
  if (!/^[0-9+\s-]{6,15}$/.test(phone)) missing.push("เบอร์โทร");
  if (!locationText) missing.push("สถานที่/โรงแรม");
  if (!service) missing.push("บริการ (พิมพ์ชื่อบริการให้ชัดเจนกว่านี้)");
  ...
  if (missing.length > 0) {
    lines.push(
      "",
      `❌ ข้อมูลไม่ครบ: ${missing.join(", ")}`,
      "พิมพ์ข้อความใหม่พร้อมข้อมูลที่ขาด แล้วส่งอีกครั้งค่ะ"
    );
...
async function handleOrderCallback(
  ...
  const draft = snap.data() as OrderDraft;

  if (action === "cancel") {
    if (draft.status === "pending") await ref.update({ status: "cancelled" });
  ...
  try {
    ...
    const mapUrl = draft.locationText
      ...
      : null;
    const { therapistShare, shopShare } = stampSplitServer(
      draft.serviceId,
      draft.servicePrice,
      draft.duration
    );
    ...
    if (msgChatId && msgId) {
      await editTelegramMessage(
        ...
        msgChatId,
        msgId,
        `✅ สร้างออเดอร์แล้ว · ${bookingCode}\n${draft.customerName} · ${draft.serviceName} · ${draft.time}`
      );
    }
  } catch (err) {
    logger.error("[handleOrderCallback] confirm failed", err);
...
function formatOpenJob(bookingId: string, b: BookingDocLite): string {
  const refCode = `SR-${bookingId.slice(0, 8).toUpperCase()}`;
  return [
    `🆕 งานว่าง · ${refCode}`,
    "",
...
const openJobKeyboard = (bookingId: string): InlineKeyboard => [
 ...
 * hotel room.
 */
async function handleOpenJobClaim(
  q: TelegramCallbackQuery,
  bookingId: string,
  token: string
): Promise<void> {
  ...
  const therapist = therapistDoc.data() as { name?: string; uid?: string };

  const ref = db.collection("bookings").doc(bookingId);
  type ClaimOutcome = "won" | "taken" | "gone";
  ...
  try {
    ...
    outcome = await db.runTransaction<ClaimOutcome>(async (tx) => {
    ...
    logger.error("[openJobClaim] transaction failed", { bookingId, err });
  ...
  if (chanId && msgId) {
    await editTelegramMessage(
      ...
      chanId,
      msgId,
      `✅ งานนี้มีคนรับแล้ว · SR-${bookingId.slice(0, 8).toUpperCase()}`
    );
  }
  ...
  logger.info("[openJobClaim] claimed", { bookingId, therapistId: therapistDoc.id });
}
...
async function handleJobCallback(
  ...
  try {
    ...
    let assignedChatId: string | null = null;
    if (b.therapistId) {
      const tSnap = await db.collection("therapists").doc(b.therapistId).get();
      const t = tSnap.data() as { telegramChatId?: string | number } | undefined;
    ...
    if (!assignedChatId || String(pressedBy).trim() !== assignedChatId) {
      logger.warn("[jobCallback] presser is not the assigned therapist", {
        bookingId,
        pressedBy,
    ...
    if (b.therapistResponse) {
      ...
      await answerCallback(
        token,
        q.id,
        wasAccepted ? "งานนี้กดรับไปแล้วค่ะ" : "งานนี้กดไม่รับไปแล้วค่ะ"
      );
      if (msgChatId && msgId) {
        await editTelegramMessage(
          token,
          msgChatId,
          msgId,
          `${wasAccepted ? "✅ รับงานแล้ว" : "❌ ไม่รับงาน"} · ตอบไปก่อนหน้านี้แล้ว`
        );
      }
      return;
    }
    ...
    logger.info("[jobCallback] recorded", { bookingId, action, pressedBy });
  } catch (err) {
    logger.error("[jobCallback] failed", err);
...
interface TelegramUpdate {
  // 🆕 28x.64 — button presses arrive as callback_query, not message. Note the
  //   webhook must also be registered with allowed_updates including
  //   "callback_query" or Telegram never delivers these at all.
  callback_query?: TelegramCallbackQuery;
  // 🆕 28x.71 — posts in the practitioner job CHANNEL arrive here, not as
  //   `message`. Used once, so the bot can learn the channel's id itself
  //   instead of the founder having to dig a negative 13-digit number out of
  //   Telegram's internals.
  channel_post?: {
  ...
  message?: {
    // 🆕 28x.63 — `type` matters: with privacy mode on, the bot still receives
  ...
  async (req, res) => {
    ...
    try {
      const linked = await getFirestore()
        ...
        .get();
      if (!linked.empty) {
        const d = linked.docs[0].data() as { name?: string };
      ...
      logger.warn("[telegramWebhook] linked-therapist lookup failed", err);
    }
...
export const createTherapistLinkCode = onCall(
  { region: "asia-southeast1" },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "Sign in required.");
...
//   job board.
export const createAdminLinkCode = onCall(
  { region: "asia-southeast1" },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "Sign in required.");
...
async function collectBusyTherapists(): Promise<Map<string, Timestamp | null>> {
  const db = getFirestore();
  const snap = await db
    .collection("bookings")
  ...
  for (const d of snap.docs) {
    ...
    if (NOT_REALLY_BOOKED.has(String(b.status ?? "").toLowerCase())) continue;

    const startMs = b.startAt instanceof Timestamp ? b.startAt.toMillis() : null;
    if (
      b.dispatchState === "assigned" &&
      startMs != null &&
    ...
    let end: Timestamp | null = b.expectedEndAt ?? b.endAt ?? null;
    if (!end && startMs != null && typeof b.duration === "number" && b.duration > 0) {
      end = Timestamp.fromMillis(startMs + b.duration * 60_000);
    }
...
async function stampTherapistUid(
  bookingId: string,
...
function reviewTs(v: unknown): number {
...
function writtenReviewOf(bookingId: string, b: ReviewBookingLite | undefined) {
  if (!b || typeof b.rating !== "number" || b.rating < 1) return null;
...
async function recomputeTherapistReviewStats(therapistId: string): Promise<void> {
  const db = getFirestore();
  const tRef = db.collection("therapists").doc(therapistId);
  const [snap, tSnap] = await Promise.all([
    db.collection("bookings").where("therapistId", "==", therapistId).get(),
  ...
  const pubList: NonNullable<ReturnType<typeof writtenReviewOf>>[] = [];
  let sum = 0;
  snap.forEach((d) => {
    const r = writtenReviewOf(d.id, d.data() as ReviewBookingLite);
    if (!r) return;
    sum += r.rating;
    pubList.push(r);
  });
  ...
  const bayes2 = reviews
    ? Math.round(
        ((REVIEW_PRIOR_MEAN * REVIEW_PRIOR_WEIGHT + sum) / (REVIEW_PRIOR_WEIGHT + reviews)) * 100
      ) / 100
    : 0;
  ...
  logger.info("[recomputeTherapistReviewStats] synced", { therapistId, rating, reviews });
}

export const onBookingWriteSyncReviews = onDocumentWritten(
  {
    document: "bookings/{bookingId}",
    region: "asia-southeast1",
  },
  async (event) => {
    const bookingId = event.params.bookingId;
    const before = event.data?.before.data() as ReviewBookingLite | undefined;
    const after = event.data?.after.data() as ReviewBookingLite | undefined;
    ...
    for (const tid of targets) {
      try {
        ...
        logger.warn("[onBookingWriteSyncReviews] recompute failed", { therapistId: tid, err });
      }
    }
  }
);
...
// ─────────────────────────────────────────────────────────────────────
export const ensureWebPushKeys = onCall(
  { region: "asia-southeast1" },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "Sign in required.");
    ...
    logger.info("[ensureWebPushKeys] generated new VAPID pair", {
      by: request.auth.uid,
    });
