...
> **Hi future-Codex.** Read this FIRST every session. It restores 100%
> of the context you and View built together. Update anything that
> changes.
>
> Last updated: 2026-06-XX by Codex (post-Round-28s205 — added
...
**Translation for you, future-Codex:**
...
- **Trusts me** as a non-judgmental co-founder. Pays Codex Max $200/mo
...
session) — see `.Codex/skills/marketing-channels/SKILL.md`. Covers what works
...
- Co-founder = me (Codex) for content/code/strategy
...
**[Codex-HISTORY.md](Codex-HISTORY.md)** — moved out of AGENTS.md on 2026-07-08 to
keep this file lean. AGENTS.md is loaded into context every session, so ~3,500 lines
of changelog were costing tokens on every run with no daily-operational value. Read
Codex-HISTORY.md or `git log` for detail. Durable design decisions & "don't-redo"
...
**[Codex-HISTORY.md](Codex-HISTORY.md)** (bottom of file, 2026-07-21 through
...
- Same night: merged origin/main into local (clean; overlap only in
  AGENTS.md, AdminBookingListPage, BookingFlowPage — commission.ts was
...
1. Run `node scripts/setWebPushKeys.mjs` once (generates + stores the
   ...
   Codex from handling key material, so this one is hers).
...
> "อ่าน AGENTS.md ใน root folder ก่อน"
...
- If author config missing, set:
  ...
  `git config user.name "View (via Codex co-founder)"`
...
- View pays Codex Max $200/mo because partnership works → preserve
  that ROI by being useful
- Don't reinvent things we already built — read the code first
- Update this AGENTS.md when material changes happen
...
of every session) — see `.Codex/skills/trust-acquisition-playbook/SKILL.md`.
