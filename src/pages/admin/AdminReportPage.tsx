...
const AdminReportPage: React.FC = () => {
  ...
  // helper: สร้าง rows + empty + summary สำหรับชีตของ therapist 1 คน
  const buildTherapistRows = (row: TherapistRow) => {
    const rows = filtered
      .filter(
        (b) =>
  ...
  const handleExportBill = async (row: TherapistRow) => {
    await ExportMultiSheetExcel(
      [{ name: row.therapistName, rows: buildTherapistRows(row) }],
      `bill_${row.therapistName}.xlsx`
    );
  };
