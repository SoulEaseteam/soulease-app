// TherapistDetailPage.tsx - Completed Version with services, features, and gallery
...
import {
  ...
  FaSmoking, FaSyringe, FaHeartbeat,FaMagic, FaSprayCan, FaAtom, FaFireAlt, FaLeaf,FaHeart,FaSpa,FaChessQueen,FaHands,FaPaintBrush, FaGem,FaPassport,FaAirFreshener,FaHotjar,FaRuler,FaPenFancy,
} from 'react-icons/fa';
...
const handleShare = async () => {
  ...
  try {
    if (navigator.share) {
      await navigator.share({ title: therapists.name, url });
    } else {
...
const TherapistDetailPage: React.FC = () => {
  ...
  const therapist = therapists.find((t) => t.id === id); // ✅ แล้วใช้ therapist.name
  const [section, setSection] = React.useState<'services' | 'features' | 'profile'>(defaultSection);
  ...
  const galleryImages = therapist.gallery || Array.from({ length: 6 }).map((_, index) => `/images/gallery/${id}_${index + 1}.jpg`);

  return (
    <Box sx={{ background: 'rgba(255, 255, 255, 0.6)', color: '#2b3b53', minHeight: '100vh', display: 'flex', justifyContent: 'center' }}>
...
>
  ...
  >
    <ShareIcon />
        ...
        {/* Avatar and Info */}
       {/* Avatar และ Favorite button ซ้อนกัน */}
<Box sx={{ px: 2, mt: -6, position: 'relative', display: 'inline-block' }}>
 <Avatar
  ...
  imgProps={{
    style: {
      objectFit: 'cover',
      objectPosition: 'center top', // ✅ แสดงช่วงบนสุด (ใบหน้า)
    },
  }}
/>
 <Button
...
>
  {therapist.name}
        ...
        >
          ...
          <Typography variant="body2" sx={{ color: '#596a7c' }}> WorkingHours {therapist.startTime} - {therapist.endTime} </Typography>
  ...
  sx={{
    ...
    py: 1,
    borderRadius: 4,
    background: 'linear-gradient(to right, #2e3a4f, #0f1113)', // เปลี่ยนสีแท็บ
...
{section === 'features' && features && (
  ...
  >
    ...
    {renderFeature(<Face color="#2b3b53"/>, 'Age', features.age)}
    {renderFeature(<FaRuler color="#2b3b53" />, 'Height', features.height)}
    {renderFeature(<FaWeight color="#2b3b53" />, 'Weight', features.weight)}
    {renderFeature(<FaVenusMars color="#2b3b53" />, 'Gender', features.gender)}
    {renderFeature(<FaFlag color="#2b3b53" />, 'Ethnicity', features.ethnicity)}
    {renderFeature(<FaPassport color="#2b3b53" />, 'Language', features.language)}
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.15 }}>
            ...
            >
              ...
              fontFamily="Playfair Display">{svc.name}</Typography>
              <Typography
                fontSize={13}
                sx={{
                  mt: 0.5,
                  ...
                  minHeight: 36,
                  pr: 17, // ✅ เพิ่ม padding ขวา ไม่ให้ชนปุ่ม Select
                }}
              >
                {svc.desc}
              </Typography>
